...
pub fn day5_part1() -> Result<()> {
    let contents =
        read_to_string("/Users/leahpeker/workspace/advent_of_code/aoc_2023/src/day5.txt").unwrap();

    let re = Regex::new(
        r"seeds:\s*([\d\s]+)|seed-to-soil map:\s*([\d\s]+)|soil-to-fertilizer map:\s*([\d\s]+)|fertilizer-to-water map:\s*([\d\s]+)|water-to-light map:\s*([\d\s]+)|light-to-temperature map:\s*([\d\s]+)|temperature-to-humidity map:\s*([\d\s]+)|humidity-to-location map:\s*([\d\s]+)",
    )?;
...
pub fn day5_part2() -> Result<()> {
    ...
    for (cap_i, cap) in cap_iter.enumerate() {
        ...
        if cap_i == 0 {
            let initial_seed_list: Vec<i64> = string
                .split_whitespace()
                .flat_map(|num| num.parse::<i64>().ok())
                .collect();
